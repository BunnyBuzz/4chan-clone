...
__name(json, "json");
function getDb(env2) {
  return env2 && env2.DB ? env2.DB : null;
}
__name(getDb, "getDb");
function newId() {
  try {
...
function parseImages(v) {
  try {
    ...
    return Array.isArray(a) ? a.filter((x) => typeof x === "string" && x.startsWith("http")).slice(0, 4) : [];
  } catch (e) {
    return [];
  }
}
...
__name(replyRow, "replyRow");

// api/threads/[id]/replies.js
async function onRequestGet(context2) {
  try {
    const db = getDb(context2.env);
    if (!db) return json({ error: "DB not bound." }, 500);
    const replies = await db.prepare("SELECT * FROM replies WHERE thread_id = ? ORDER BY created_at ASC").bind(context2.params.id).all();
...
async function onRequestPost(context2) {
  try {
    const db = getDb(context2.env);
    if (!db) return json({ error: "DB not bound." }, 500);
    const threadId = context2.params.id;
    ...
    const images = Array.isArray(body.images) ? body.images.filter((u) => typeof u === "string" && u.startsWith("http")).slice(0, 4) : [];
    if (!text && !images.length) return json({ error: "Empty reply." }, 400);
    if (text.length > 2e3) return json({ error: "Text too long." }, 400);
    const id = newId();
    const now = Math.floor(Date.now() / 1e3);
    await db.prepare("INSERT INTO replies (id, thread_id, author, text, images, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(id, threadId, author, text, JSON.stringify(images), now).run();
    return json(
...
async function onRequestPost2(context2) {
  try {
    const { filename } = await context2.request.json();
    const userhash = context2.env.CATBOX_USERHASH;
    ...
    catboxData.append("files", filename);
...
async function onRequestGet3(context2) {
  try {
    ...
    const rows = await context2.env.DB.prepare(
      "SELECT t.id, t.board, t.author, t.text, t.images, t.created_at, (SELECT COUNT(*) FROM replies r WHERE r.thread_id = t.id) AS replies FROM threads t WHERE t.board = ? ORDER BY t.created_at DESC LIMIT ?"
...
async function onRequestPost3(context2) {
  try {
    ...
    if (text.length > 5e3) return json({ error: "Text too long." }, 400);
    const id = newId();
    const now = Math.floor(Date.now() / 1e3);
    await db.prepare("INSERT INTO threads (id, board, author, text, images, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(id, board, author, text, JSON.stringify(images), now).run();
    return json(
      { thread: { id, board, author, text, images, created_at: now, replies: 0 } },
      201
    );
  } catch (e) {
    return json({ error: e.message }, 500);
...
// ../.wrangler/tmp/pages-9E1LDy/functionsRoutes-0.8498305554851654.mjs
var routes = [
  {
