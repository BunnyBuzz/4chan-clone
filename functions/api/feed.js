...
export async function onRequestGet(context) {
  try {
    ...
    const cols = 't.id, t.board, t.author, t.user_id, t.subject, t.tags, t.views, t.text, t.images, t.created_at, ' +
    ...
    const rows = board === 'all'
      ? await db.prepare(
        'SELECT ' + cols + 'FROM threads t ORDER BY t.created_at DESC LIMIT ?'
      ).bind(limit).all()
      : await db.prepare(
        'SELECT ' + cols + 'FROM threads t WHERE t.board = ? ORDER BY t.created_at DESC LIMIT ?'
